    ...
    <nav class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex space-x-6 text-sm font-medium">
                ...
                <a href="/pemesanans" class="text-blue-600 border-b-2 border-blue-600 pb-1">Data Pemesanan</a>
            </div>
        </div>
    </nav>
    ...
    <main class="max-w-6xl mx-auto px-6 py-8">
        <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800">Data Pemesanan</h1>
                    <p class="text-sm text-gray-500">Sistem Pemesanan Layanan SMKN 1 Katapang</p>
                </div>
                <a href="/pemesanans/create" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition inline-block">
                    + Tambah Pemesanan
                </a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700">
                        @foreach ($pemesanans as $index => $pemesanan)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="py-3 px-4 font-medium text-gray-900">{{ $pemesanan->pelanggan->nama_pelanggan ?? '-' }}</td>
                                <td class="py-3 px-4 text-gray-700">{{ $pemesanan->layanan->nama_layanan ?? '-' }}</td>
                                <td class="py-3 px-4 text-gray-500">{{ $pemesanan->created_at->format('d M Y') }}</td>
